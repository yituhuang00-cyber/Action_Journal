...
import { getGoal, updateGoal } from '../storage/storage'
...
export default function EditGoal() {
  ...
  useEffect(() => {
    ...
    setExpectedOutcome(goal.expectedOutcome || '')
    setSupports(goal.supports && goal.supports.length ? goal.supports : [''])
    setFactors(goal.factors && goal.factors.length ? goal.factors : [{ name: '', controllability: 5 }])
    setLoading(false)
  }, [id, navigate])
  ...
  function removeSupport(i) {
    const next = supports.filter((_, idx) => idx !== i)
  ...
  function removeFactor(i) {
    const next = factors.filter((_, idx) => idx !== i)
    setFactors(next.length ? next : [{ name: '', controllability: 5 }])
  }
  ...
  function handleSave(e) {
    ...
    const payload = {
      ...
      factors: factors
        ...
        .map((f) => ({
          name: f.name.trim(),
          controllability: Number(f.controllability),
        })),
    }
    ...
    <div className="page new-goal-page">
      ...
      <form onSubmit={handleSave} style={{ maxWidth: 760 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          ...
          <button type="button" className="small-btn ghost" onClick={() => navigate('/goals')}>取消</button>
        </div>
      </form>
    </div>
  )
